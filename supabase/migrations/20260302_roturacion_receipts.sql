-- 20260302_roturacion_receipts.sql
-- Add receipt and signature columns to roturacion_ejecuciones
...
-- RPC Function for Technician Signature with Workflow Guard
...
BEGIN
    ...
    IF v_user_role NOT IN ('tecnico', 'admin') THEN
        RAISE EXCEPTION 'Acceso denegado: Se requiere rol de técnico para firmar.';
    END IF;

    -- 3. Get Execution Details and validate State
    -- We need to check if the labor is marked as TERMINADO in roturacion_seguimiento
    SELECT 
        rs.id,
        ra.labor,
        CASE 
    ...
    IF v_estado_actual != 'TERMINADO' THEN
        RAISE EXCEPTION 'La labor debe estar en estado TERMINADO para ser firmada.';
    END IF;
